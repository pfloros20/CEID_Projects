...
__global__ void covariance_mean(double* data, double* mean)
{
...
__global__ void covariance_center(double* data, double* mean)
{
	int j = threadIdx.x + blockDim.x * blockIdx.x;
	int i;
	if(j<M){
		double loc_mean = mean[j];
	  	/* Center the column vectors. */
		for (i = 0; i < N; i++) 
			data[i*M + j] -= loc_mean;
	}
}
__global__ void covariance_kernel(double* data, double* symmat)
{
	int j = threadIdx.x + blockDim.x * blockIdx.x;
	int i,j1;
	double temp = 0;
...
int main(int argc, char *argv[])
{
	...
	//Initialize data
	init_arrays(data);
	data_h = (double*)memcpy(data_h,data,M*N*sizeof(double));
	...
	//Kernel Call
	covariance_mean <<< gridDim, blockDim >>>(data_d, mean_d);
	covariance_center <<< gridDim, blockDim >>>(data_d, mean_d);
	covariance_kernel <<< gridDim, blockDim >>>(data_d, symmat_d);
