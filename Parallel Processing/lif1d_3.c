...
int main(int argc, char *argv[])
{
	...
	for (i = 0; i < n; i++) {
		...
		long int size=ntstep*(n*(8)+8);
		char *buff1;
		buff1=malloc(size);
		char *buff2;
		buff2=malloc(size);
		setvbuf(output1,buff1,_IOFBF,size);
		setvbuf(output2,buff2,_IOFBF,size);
	#endif
	...
	double dtmu=dt*mu;
	double oneminusdt=1-dt;
	double sigmasum[n];
	#pragma omp parallel for
	for(i=0;i<n;i++)
		cblas_dscal(n,dt/divide,&sigma[i*n],1);
	#pragma omp parallel for
	for(i=0;i<n;i++)
		sigmasum[i]=cblas_dasum(n,&sigma[i*n],1);
	...
	for (it = 0; it < itime; it++) {
		...
		for (i = 0; i < n; i++) {
			 ...
			 */
			sum=cblas_ddot(n,&sigma[i*n],1,u,1)-sigmasum[i]*u[i];
			uplus[i] += sum;
...
#if defined(ALL_RESULTS)
			fwrite(&it,sizeof(long),1,output1);
			fwrite(u,sizeof(double),n,output1);
			 for (i = 0; i < n; i++) {
			 	omega[i] = 2.0 * M_PI * omega1[i] / (time - ttransient * dt);
			 }
			fwrite(omega,sizeof(double),n,output2);

			if(it % ntstep == 0 ||it == itime-1){
				fflush(output1);
				fflush(output2);
			}
#else
